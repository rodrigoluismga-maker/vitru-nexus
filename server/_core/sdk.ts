...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    ...
    if (!ENV.oAuthServerUrl) {
      console.error(
        "[OAuth] ERROR: OAUTH_SERVER_URL is not " +
          "configured! Set OAUTH_SERVER_URL environment variable."
      );
    }
  }
...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (user && user.status !== "active") {
      ...
      await db.recordAuthenticationDenied(user.id, reason);
      throw ForbiddenError("User access is not authorized");
    }
    ...
    db.assertUserCanAuthenticate(user);
    await db.upsertUser({
      openId: user.openId,
      lastSignedIn: signedInAt,
    });
 ...
 * See the project documentation for periodic updates.
 */
